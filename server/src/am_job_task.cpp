...
_get_dir_list (list < string > &files_list, const char *roor_dir, string special_key)
{
  ...
  while (FindNextFile (handle, &ffd))
	...
	{
	  ams_log_name = ffd.cFileName;
	  if (special_key.length() == 0)
	    {
	      files_list.push_back (ams_log_name);
	    }
	  else
	    {
	      if (ams_log_name.find (special_key) < ams_log_name.length())
		{
		  files_list.push_back (ams_log_name);
		}
	    }
...
ts_load_access_log (nvplist *req, nvplist *res, char *_dbmt_error)
{
  ...
  string log_exp = ".log";
  string err_exp = ".err";
  string ams_process_name = AMS_NAME;
  ...
  _get_dir_list (all_files_list, root_dir, ams_process_name);
  ...
  if (log_files_list.empty() == true)
    {
      ...
      snprintf (log_full_dir, PATH_MAX, "%s%s%s", root_dir, ams_process_name.c_str(), log_exp.c_str());
...
{
  int retval = ERR_NO_ERROR;
  T_AMS_HOST_STAT stat;

  memset (&stat, 0, sizeof (T_AMS_HOST_STAT));
...
int
ts_get_ams_env (nvplist *req, nvplist *res, char *_dbmt_error)
{
...
ts_get_shard_info (nvplist *req, nvplist *res, char *_dbmt_error)
{
  ...
  /* not thread safe :( */
  reqid++;
  sprintf (stdout_log_file, "%s/amshardinfo.%d.err", sco.dbmt_tmp_dir, reqid);
  sprintf (stderr_log_file, "%s/amshardinfo2.%d.err", sco.dbmt_tmp_dir, reqid);
...
ts_get_shard_status (nvplist *req, nvplist *res, char *_dbmt_error)
{
  ...
  /* not thread safe :( */
  reqid++;
  sprintf (stdout_log_file, "%s/amshardstatus.%d.err", sco.dbmt_tmp_dir, reqid);
  sprintf (stderr_log_file, "%s/amshardstatus2.%d.err", sco.dbmt_tmp_dir, reqid);
...
ts_shard_start (nvplist *req, nvplist *res, char *err_buf)
{
  ...
  sprintf (stdout_log_file, "%s/amshardstart.%d.err", sco.dbmt_tmp_dir,
	   (int) getpid ());
  sprintf (stderr_log_file, "%s/amshardstart2.%d.err", sco.dbmt_tmp_dir,
	   (int) getpid ());
...
ts_shard_stop (nvplist *req, nvplist *res, char *err_buf)
{
  ...
  sprintf (stdout_log_file, "%s/amshardstop.%d.err", sco.dbmt_tmp_dir,
	   (int) getpid ());
  sprintf (stderr_log_file, "%s/amshardstop2.%d.err", sco.dbmt_tmp_dir,
	   (int) getpid ());
...
_write_auto_update_log (char *line_buf, int is_success)
{
  ...
  sprintf (log_path, "%s/log/manager/ams.update.log", sco.szArniadb);
  ...
  if (is_success)
    ...
    {
      fprintf (fin, "[%s] AMS update error: %s\n", log_time, line_buf);
    }
...
ts_is_update_success (nvplist *req, nvplist *res, char *_dbmt_error)
{
  ...
  sprintf (err_log, "%s/ams_auto_update.err", sco.dbmt_tmp_dir);
...
ts_auto_update (nvplist *req, nvplist *res, char *_dbmt_error)
{
  ...
  sprintf (err_log, "%sams.autoupdate.err", path);
  sprintf (output_log, "%sams.autoupdate.log", path);
