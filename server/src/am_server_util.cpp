...
static T_FSERVER_TASK_INFO task_info[] =
{
  ...
  {"getamsenv", TS_GET_AMS_ENV, 0, DEF_TASK_FUNC (ts_get_ams_env), FSVR_SA_CS, ALL_AUTHORITY},
...
int
is_amserver_process (int pid, const char *module_name)
{
#if defined(WINDOWS)
...
ut_validate_token (nvplist *req)
{
  ...
  if (!strcmp (nv_get_val (req, "task"), "login") ||
      !strcmp (nv_get_val (req, "task"), "getamsenv"))
    {
      ut_access_log (req, "task = login or getamsenv.");
      return 1;
    }
...
ut_validate_auth (nvplist *req)
{
  ...
  if (!strcmp (task, "login") || !strcmp (task, "getamsenv"))
    {
      return 1;
    }
...
int
ut_get_proc_stat (T_AMS_PROC_STAT *stat, int pid)
{
...
int
ut_get_host_stat (T_AMS_HOST_STAT *stat, char *_dbmt_error)
{
