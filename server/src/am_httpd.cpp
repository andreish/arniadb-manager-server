...
stop_service ()
{
  ...
  conf_get_dbmt_file (FID_AMSERVER_PID, arn_manager_pid_file);
  if (access (arn_manager_pid_file, F_OK) < 0)
  ...
  else
    {
      ...
      if ((pidfile == NULL) || ((kill (pidnum, SIGTERM)) < 0))
        ...
        {
          unlink (conf_get_dbmt_file (FID_AMSERVER_PID, arn_manager_pid_file));
...
get_processid ()
{
  ...
  conf_get_dbmt_file (FID_AMSERVER_PID, pid_file_name);
...
init_files ()
{
  ...
  unlink (conf_get_dbmt_file (FID_AMSERVER_PID, strbuf));
}
...
main (int argc, char **argv)
{
  ...
  if (ut_write_pid (conf_get_dbmt_file (FID_AMSERVER_PID, dbmt_file)) < 0)
    {
      snprintf (tmpstrbuf, DBMT_ERROR_MSG_SIZE,
