...
static T_DBMT_FILE_INFO dbmt_file[NUM_DBMT_FILE] =
{
  ...
  {FID_AMSERVER_PID, DBMT_PID_DIR, DBMT_ARN_AMS_PID},
  {FID_AMS_LOG, DBMT_LOG_DIR, "arn_manager.log"},
  {FID_AMS_ERROR_LOG, DBMT_LOG_DIR, "arn_manager.err"},
...
uReadSystemConfig (void)
{
  ...
  sco.iAMS_port = DEFAULT_AMS_PORT;
  ...
  strcpy (sco.szAMSVersion, "");
  strncpy (sco.szTokenActiveTime, "7200", PATH_MAX);
  snprintf (sco.szCWMPath, PATH_MAX, "%s%s", sco.szArniadb,
            DEFAULT_CWM_PATH_SHORT);

  conf_get_dbmt_file (FID_AMS_LOG, access_log_buf);
  conf_get_dbmt_file (FID_AMS_ERROR_LOG, error_log_buf);
  ...
  while (fgets (cbuf, sizeof (cbuf), conf_file))
    {
      ...
      */
      ...
      if (strcasecmp (ent_name, "cm_port") == 0)
        ...
        {
          snprintf (sco.szAMSVersion, PATH_MAX, "%s", ent_val);
        }
